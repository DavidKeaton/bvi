...
void usage(void)
{
    static char msg[] = "Usage: %s [-R] [-c cmd | +cmd] [-f script]\n\
...
{
    int ch, lflag, n = 1, script = -1;
    long count;
    off_t inaddr;
#ifdef HAVE_LOCALE_H
    // TODO: make sure this is proper, had issues with this in other project
    setlocale(LC_ALL, "");
#endif
    char *poi = strrchr(argv[0], DELIM);
    strncpy(progname, (poi) ? ++poi : argv[0], 7);
    strtok(progname, "."); // TODO: wtf is this for? if called from local directory?
                ...
                do {
                    ...
                    */
                    if(x < (Anzahl3 + AnzAdd - 2)) {
                        x += 3;
                    } else if(x > (Anzahl3 + 3) && x < (Anzahl3 + AnzAdd - 1 + Anzahl)) {
                        x++;
                    }
                } while(--precount > 0);
                if(x < AnzAdd + Anzahl3) {
...
int do_append(off_t count, char *buf)
{
...
void do_undo(void)
{
    ...
    switch(edits) {
        ...
        case U_INSERT:
            ...
            memmove(undo_start, undo_start + undo_count, maxpos - undo_start);
            edits = U_DELETE;
            break;
        case U_BACK:
        case U_DELETE:
            filesize += undo_count;
            maxpos += undo_count;
            memmove(undo_start + undo_count, undo_start, maxpos - undo_start);
    ...
    switch(ch1) {
        ...
        case 'G':
            if(count == -1) {
                trunc_cur();
                return 0;
            } else if((count < P(P_OF)) || (count - (off_t)P(P_OF)) > (filesize - 1L)) {
                beep();
                return 0;
            } else {
