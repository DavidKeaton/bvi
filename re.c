...
/* You cannot use a common regexp subroutine, because 
 * \0 is a regular character in a binary string! */
#include    "bvi.h"
#include    "set.h"
...
PTR bregexec(PTR start, char *scan)
{
    char *act;
    int count, test;
    act = act_pat;
...
PTR fsearch_end(PTR start, PTR end, char *smem, PTR *s_end)
{
    PTR spos;
